...
def get_window_split(og_mask_vec: list[bool], seqid_vec: list[int], gene_idx: int, window_size: int, is_circular: bool) -> tuple[list[int], list[int]]:
    """Retrieve left and right neighbourhood gene indices separately.

    Same scan logic as :func:`get_window` but returns ``(left_indices, right_indices)`` so
    callers can determine which side of a contig edge is truncated.

    Args:
        og_mask_vec (list[bool]): Boolean mask — True when the gene belongs to the target OG.
        seqid_vec (list[int]): Sequence/scaffold ID per gene in the genome.
        gene_idx (int): Index of the focal gene.
        window_size (int): Total window size (left + right = window_size).
        is_circular (bool): Whether the genome is treated as circular.

    Returns:
        tuple[list[int], list[int]]: ``(left_indices, right_indices)`` in ascending order.
    """
    ...
...
def calculate_directional_synteny_ratio(
    left_a: list[int],
    right_a: list[int],
    left_b: list[int],
    right_b: list[int],
    ...
    """Directional synteny ratio that respects contig-edge truncations.

    Compares only the flanks that are available for both genes.  When neither gene
    has any edge flags the behaviour is identical to :func:`calculate_synteny_ratio`
    (merged windows, max denominator).  When at least one gene is at an edge the
    comparison is restricted to the side(s) that both genes can provide.

    Args:
        left_a (list[int]): OG indices from the left flank of gene A (need not be sorted).
        right_a (list[int]): OG indices from the right flank of gene A.
        left_b (list[int]): OG indices from the left flank of gene B.
        right_b (list[int]): OG indices from the right flank of gene B.
        left_edge_a (bool): True when gene A is missing left context (contig edge).
        right_edge_a (bool): True when gene A is missing right context (contig edge).
        left_edge_b (bool): True when gene B is missing left context (contig edge).
        right_edge_b (bool): True when gene B is missing right context (contig edge).

    Returns:
        float: Directional synteny ratio in [0, 1].
    """
    ...
